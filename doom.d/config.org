...
(defun nm/org-capture-to-task-file ()
  ...
  (let* ((file "~/Documents/org/gtd/tasks.org")
         (parent-l nil)
         (child-l nil)
         (parent "Daily Tasks")
         (date (org-read-date))
...
#+begin_src emacs-lisp
(setq org-capture-templates '(("!" "Quick Task" checkitem (file+olp "~/Documents/org/gtd/tasks.org" "Tasks") "- [ ] %?")))
#+end_src
...
(push '("i" "Capture to inbox" entry (file+olp "~/Documents/org/gtd/inbox.org" "Inbox") "* TODO %^{task}\n:PROPERTIES:\n:CREATED: %U\n:END:\n%^{Why are we capturing?}") org-capture-templates)
(push '("t" "Capture Task with Link" entry (file+olp "~/Documents/org/gtd/inbox.org" "Inbox") "* TODO %^{task} %a\n:PROPERTIES:\n:CREATED: %U\n:END:\n\n%i") org-capture-templates)
...
#+begin_src emacs-lisp
(push '("r" "research literature" entry (file+function "~/Documents/org/gtd/websources.org" nm/enter-headline-websources) "* READ %(get-page-title (current-kill 0))") org-capture-templates)
...
(push '("r" "Research"
        ((todo ""
               ((org-agenda-files (append (file-expand-wildcards "~/Documents/org/gtd/literature.org")))
